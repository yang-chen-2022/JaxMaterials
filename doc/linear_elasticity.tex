...
\begin{equation}
    \begin{aligned}
        ...
        \sigma_{ij} &= C_{ijk\ell} \varepsilon_{k\ell}\qquad\text{with $\varepsilon_{k\ell} = \varepsilon^*_{k\ell} + E_{k\ell}$}\\
...
\end{equation}
$E$ is a constant symmetric tensor and $u=u(x)$ is periodic such that $\int_\Omega u_i(x)\;dx = 0$ and $\int_\Omega \partial_i u_j(x)\;dx = 0$, i.e. $\int_\Omega \varepsilon_{ij}(x)\;dx=0$.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
location & variable & \\
\hline\hline
\multirow{2}{*}{voxel centres $\mathcal{C}$} & strain & $\varepsilon_{ij}=\frac{1}{2}(\partial_i u_j + \partial_j u_i)$ \\
& stress & $\sigma_{ij}=C_{ijk\ell} \varepsilon_{k\ell}$ \\
...
\end{aligned}
\end{equation}
If we use Voigt's notation \cite{voigt1928lehrbuch} to arrange the six independent components of the symmetric $3\times 3$ tensors $\widehat{\varepsilon}$ and $\widehat{\tau}$ into vectors
\begin{xalignat}{2}
...
\begin{aligned}    
    ...
    \xir_0^2 \widehat{\tau}_0 + \xir_0(\xir_2\widehat{\tau}_4+\xir_1\widehat{\tau}_5)\\[2ex]
    \xir_1^2 \widehat{\tau}_1 + \xir_1(\xir_2\widehat{\tau}_3+\xir_0\widehat{\tau}_5)\\[2ex]
    \xir_2^2 \widehat{\tau}_2 + \xir_2(\xir_1\widehat{\tau}_3+\xir_0\widehat{\tau}_4)\\[2ex]
    \frac{1}{2}\left(\xir_1\xir_2(\widehat{\tau}_1+\widehat{\tau}_2)+(\xir_1^2+\xir_2^2)\widehat{\tau}_3+\xir_0(\xir_1\widehat{\tau}_4+\xir_2\widehat{\tau}_5)\right)\\[2ex]
    \frac{1}{2}\left(\xir_0\xir_2(\widehat{\tau}_0+\widehat{\tau}_2)+(\xir_0^2+\xir_2^2)\widehat{\tau}_4+\xir_1(\xir_0\widehat{\tau}_3+\xir_2\widehat{\tau}_5)\right)\\[2ex]
    \frac{1}{2}\left(\xir_0\xir_1(\widehat{\tau}_0+\widehat{\tau}_1)+(\xir_0^2+\xir_1^2)\widehat{\tau}_5+\xir_2(\xir_0\widehat{\tau}_3+\xir_1\widehat{\tau}_4)\right)
    \end{pmatrix}
\end{aligned}
\end{equation}
and
...
\begin{aligned}    
    ...
    \left(
    \xir_0^2\widehat{\tau}_0+ \xir_1^2\widehat{\tau}_1+ \xir_2^2\widehat{\tau}_2+ 2(\xir_1\xir_2\widehat{\tau}_3+ \xir_0 \xir_2\widehat{\tau}_4+ \xir_0\xir_1\widehat{\tau}_5)
    \right)\begin{pmatrix}
    \xir_0^2 \\[2ex] \xir_1^2 \\[2ex] \xir_2^2 \\[2ex] \xir_1\xir_2 \\[2ex] \xir_0 \xir_2 \\[2ex] \xir_0\xir_1
    \end{pmatrix}^\top 
\end{aligned}
\end{equation}
where the expression for $\widehat{\vec{\varepsilon}}^{*(A)}=\widehat{A}\widehat{\vec{\tau}}$ follows from the $6\times 6$ matrix representation of $\widehat{A}_{k\ell ij}$
\begin{equation}
\widehat{A} = \begin{pmatrix}
    \xir_0^2 & 0 & 0 & 0 & \xir_0\xir_2 & \xir_0\xir_1 \\[2ex]
    0 & \xir_1^2 & 0 & \xir_1\xir_2 & 0 & \xir_0\xir_1\\[2ex]
    0 & 0 & \xir_2^2 & \xir_1\xir_2 & \xir_0\xir_2 & 0\\[2ex]
    0 & \frac{1}{2}\xir_1\xir_2 & \frac{1}{2}\xir_1\xir_2 & \frac{1}{2}(\xir_1^2+\xir_2^2) & \frac{1}{2}\xir_0\xir_1 & \frac{1}{2}\xir_0\xir_2\\[2ex]
    \frac{1}{2}\xir_0\xir_2 & 0 & \frac{1}{2}\xir_0\xir_2 & \frac{1}{2}\xir_0\xir_1 & \frac{1}{2}(\xir_0^2+\xir_2^2) & \frac{1}{2}\xir_1\xir_2\\[2ex]
    \frac{1}{2}\xir_0\xir_1 & \frac{1}{2}\xir_0\xir_1 & 0 & \frac{1}{2}\xir_0\xir_2 & \frac{1}{2}\xir_1\xir_2 & \frac{1}{2}(\xir_0^2+\xir_1^2)
    \end{pmatrix}
\end{equation}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{algorithmic}[1]
    ...
    \State{Update $\varepsilon_{k\ell} = \mathscr{F}^{-1}[\widehat{\varepsilon}^*_{k\ell}]+E_{k\ell}$}\Comment{Inverse Fourier-transform}        
    \EndFor
...
\begin{algorithmic}[1]
    ...
    \State{Compute $r_{ij} = \mathscr{F}[\widehat{r}_{ij}]$}\Comment{Inverse Fourier-transform}
...
\begin{algorithmic}[1]
    ...
    \EndFor
    \EndFor
    \State{Compute $r^{(0)}_{ij} = \mathscr{F}[\widehat{r}_{ij}]$}\Comment{Inverse Fourier-transform}
