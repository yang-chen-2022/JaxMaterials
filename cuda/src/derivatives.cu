                                             ...
                                             const GridSpec grid_spec)
{
  ...
  int nz = grid_spec.nz;
  __shared__ float
      _u[(BLOCKSIZE_X + 1) * (BLOCKSIZE_Y + 1) * (BLOCKSIZE_Z + 1)];
  ...
  if ((a < nx) && (b < ny) && (c < nz))
  {
    ...
    _u[IDX(BLOCKSIZE_X + 1, BLOCKSIZE_Y + 1, BLOCKSIZE_Z + 1, threadIdx.x + 1,
           threadIdx.y + 1, threadIdx.z + 1)] = u[IDX(nx, ny, nz, a, b, c)];
    if (threadIdx.x == 0)
      _u[IDX(BLOCKSIZE_X + 1, BLOCKSIZE_Y + 1, BLOCKSIZE_Z + 1, threadIdx.x,
  ...
  if ((a < nx) && (b < ny) && (c < nz))
  {
    float _du_dx = 0;
    // apply derivative
    _du_dx += _u[IDX(BLOCKSIZE_X + 1, BLOCKSIZE_Y + 1, BLOCKSIZE_Z + 1,
